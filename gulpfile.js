...
function watch() {
  ...
  gulp.watch(paths.images.src, copy, buildSw);

  gulp.watch('dist/index.html', browserSync.reload);
}
gulp.task('build-sw',buildSw);
const dist = gulp.series(gulp.parallel(copy, styles, scripts), inline, buildSw);
const dev = gulp.series(dist, watch,buildSw);
