...
const newsUrl = 'http://newsapi.org/v2/top-headlines?' +'country=us&' +'apiKey=6b3d4dd345584e7c953a11d20dd1219b';
...
const fetchTopNews = function(){
  const newsUrl = 'http://newsapi.org/v2/top-headlines?' +'country=us&' +'apiKey=6b3d4dd345584e7c953a11d20dd1219b';
  
  return fetchJson(newsUrl).then(
    data =>{
  ...
  return articles
    .map(article => {
      let articleHtml =
        ...
        <p>
          <span class="time secondary">${article.publishedAt} </span>
          published by
          <span class="by secondary"> ${article.source.name}</span>
        </p>
        <p>
       
        <img src='${article.urlToImage}'/>
        ${article.description}
        <a href=${article.url}>resource </a>
        </p>
      </section>`
      return articleHtml;
    })
...
window.addEventListener('load', () => {
  fetchTopNews()
    .then(articles => {
      const articlesHtml = renderNews(articles);
      document.getElementById('loading').style.display = 'none';
      document.getElementById('container').insertAdjacentHTML('beforeend', articlesHtml);
